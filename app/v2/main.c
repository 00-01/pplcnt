...
extern L1_CL_MEM AT_L1_POINTER
                        lynred_L1_Memory;
...
void printBboxes(bboxs_t *boundbxs) {
    ...
    for (int counter = 0; counter < boundbxs->num_bb; counter++) {
        if (boundbxs->bbs[counter].alive)
            printf("bbox [%02d] : %.5f   %03d   %03d   %03d   %03d   %02d\n", counter, FIX2FP(boundbxs->bbs[counter].score, 7), boundbxs->bbs[counter].x, boundbxs->bbs[counter].y, boundbxs->bbs[counter].w, boundbxs->bbs[counter].h,
...
//    printf("\tbodard number: 05\n");
    printf("\tflash check number: 111\n\n");
